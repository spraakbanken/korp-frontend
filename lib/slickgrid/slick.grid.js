...
(function($) {
    ...
    function SlickGrid(container,data,columns,options) {
        ...
        function styleColumnWidth(index,width,styleCells) {
            ...
            if (styleCells) {
            	var rule = findCssRuleForCell(index);
            	var newWidth = ( (width - cellWidthDiff) < 0 ? 0 : (width - cellWidthDiff)) + "px";
                rule.style.width = newWidth;
            }
        }
...
//            $("<div>").html(stringArray.join("")).each(function() {
//                rowsCache[rows[i]] = parentNode.appendChild($.firstChild);
//            });
