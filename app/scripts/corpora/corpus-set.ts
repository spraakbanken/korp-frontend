...
export class CorpusSet {
    ...
    pickFrom(source: CorpusSet, ids: string[]) {
        this.corpora = source.corpora.filter((corpus) => ids.includes(corpus.id))
        this.updateAttributes()
    }

    map<T>(f: (corpus: CorpusTransformed) => T): T[] {
    ...
    partitionAttrs(whitelist?: string[]): [string[], string[]] {
        ...
        return partition(names, (name) => CorpusSet.isStruct(attrs[name]))
    }
    ...
    corpusHasAttrs(corpusId: string, attrs: string[]): boolean {
        const corpus = this.get(corpusId)
    ...
    getWithinKeys(): string[] {
        const struct = this.map((corpus) => Object.keys(corpus.within))
        return union(...struct)
    }
    ...
    /**
     * Calculate `within` parameter for the corpora, as a comma-separated list of corpora differing from the
     * given default.
     */
    ...
    getCommonWithins(): Record<string, string> {
        // only return withins that are available in every corpus
        const allWithins = this.map((corp) => corp.within)
    ...
    getUndatedRatio(): number {
        const non_time = sum(this.map((corpus) => corpus.non_time || 0))
        const totalsize = sum(this.map((corpus) => Number(corpus.info.Size) || 0))
    ...
    updateAttributes(): void {
        const common_keys = compact(this.corpora.flatMap((corp) => Object.keys(corp.common_attributes || {})))
