...
korpApp.controller("resultContainerCtrl", ($scope, searches, $location) => $scope.searches = searches)
    ...
    constructor(scope, timeout, utils, location, kwicDownload) {
        ...
        const readingChange = function() {
            if (s.instance != null ? s.instance.getProxy().pendingRequests.length : undefined) {
                ...
                return $.when(...Array.from(s.instance.getProxy().pendingRequests || [])).then(function() {
        ...
        const massageData = function(hitArray) {
            ...
            for (let i = 0; i < hitArray.length; i++) {
                ...
                const [matchSentenceStart, matchSentenceEnd] = Array.from(findMatchSentence(hitContext))
                ...
                for (let j = 0, end1 = hitContext.tokens.length, asc = end1 >= 0; asc ? j < end1 : j > end1; asc ? j++ : j--) {
                    var structItem
                    ...
                    for (structItem of Array.from((wd.structs != null ? wd.structs.open : undefined) || [])) {
                        var key, val
                    ...
                    for (structItem of Array.from((wd.structs != null ? wd.structs.close : undefined) || [])) {
                ...
                if (prevCorpus !== id) {
                    ...
                    const newSent = { newCorpus : corpus.title, noContext : _.keys(corpus.context).length === 1 }
                    output.push(newSent)
                }
        ...
        var findMatchSentence = function(hitContext) {
            ...
            while (decr >= 0) {
                var needle
                if ((needle = "sentence", Array.from((__guard__(hitContext.tokens[decr--].structs, x => x.open) || [])).includes(needle))) {
                    span[0] = decr
                    break
                }
            }
            while (incr < hitContext.tokens.length) {
                var needle1
                if ((needle1 = "sentence", Array.from((__guard__(hitContext.tokens[incr++].structs, x1 => x1.close) || [])).includes(needle1))) {
                    span[1] = incr
                    break
                }
            }
        ...
        s.download = {
            ...
            init: (value, hitsDisplay) => {
                ...
                const [fileName, blobName] = Array.from(this.kwicDownload.makeDownload(...Array.from(value.split("/")), s.kwic, requestData, hitsDisplay))
    ...
    constructor(scope, $timeout, utils, $location, kwicDownload) {
        ...
        s.toggleReading = function() {
            ...
            if (s.instance != null ? s.instance.getProxy().pendingRequests.length : undefined) {
                ...
                return $.when(...Array.from(s.instance.getProxy().pendingRequests || [])).then(() => s.instance.makeRequest())
            }
        }
    }
        ...
        controller($scope, utils, $location, backend, searches, $rootScope) {
            ...
            return s.showMap = function() {
                ...
                for (let rowIx of Array.from(s.instance.getSelectedRows())) {
                    ...
                    const parts = Array.from(searchParams.reduceVals).map((reduceVal) =>
    ...
    ({
        controller($scope, $rootScope, $location, utils, searches) {
            ...
            $scope.parseLemgram = function(row, allLemgrams) {
              ...
              const hasHomograph = ((needle = lemgram.slice(0, -1)), Array.from(allLemgrams).includes(needle))
            ...
            $scope.getTableClass = (wordClass, parentIdx, idx) => settings.wordPictureConf[wordClass][parentIdx][idx].css_class
...
korpApp.directive("compareCtrl", () =>
    ({
        controller($scope, $rootScope) {
            ...
            return s.promise.then(function(...args) {
                const [tables, max, cmp1, cmp2, reduce] = Array.from(args[0]); const xhr = args[1]
                s.loading = false
                ...
                s.stringify = _.map(reduce, item => __guard__(attributes[_.trimStart(item, "_.")], x => x.stringify) || angular.identity)
                ...
                return s.rowClick = function(row, cmp_index) {
                    ...
                    const cqps = _.map(tokens, function(token) {
                        const cqpAnd = _.map(__range__(0, token.length - 1, true), function(attrI) {
                            ...
                            if (attribute) {
                                ({ type } = attribute)
                                if (attribute.isStructAttr) { attrKey = `_.${attrKey}` }
                            }
                            ...
                            if ((type === "set") && (attrVal.length > 1)) {
                                ...
                                _.map(attrVal, function(val) {
                                    ...
                                    if (variants.length === 0) {
                                        let asc, end
                                        for (idx = 0, end = parts.length - 2, asc = end >= 0; asc ? idx <= end : idx >= end; asc ? idx++ : idx--) {
                                            variants.push([])
                                        }
                                    }
                                    return (() => {
                                        let asc1, end1
                                        const result = []
                                        for (idx = 1, end1 = parts.length - 1, asc1 = end1 >= 1; asc1 ? idx <= end1 : idx >= end1; asc1 ? idx++ : idx--) {
