    ...
    constructor(scope, timeout, utils, location, kwicDownload) {
        ...
        const massageData = function(hitArray) {
            ...
            for (let i = 0; i < hitArray.length; i++) {
                ...
                for (let j = 0, end1 = hitContext.tokens.length, asc = 0 <= end1; asc ? j < end1 : j > end1; asc ? j++ : j--) {
                    var structItem;
    ...
    ({
        controller($scope, $rootScope) {
            ...
            return s.promise.then((function(...args) {
                const [tables, max, cmp1, cmp2, reduce] = Array.from(args[0]), xhr = args[1];
                s.loading = false;
                            ...
                            if ((type === "set") && (attrVal.length > 1)) {
                                ...
                                _.map(attrVal, function(val) {
                                    ...
                                    if (variants.length === 0) {
                                        let asc, end;
                                        for (idx = 0, end = parts.length - 2, asc = 0 <= end; asc ? idx <= end : idx >= end; asc ? idx++ : idx--) {
                                            variants.push([]);
                                        }
                                    }
                                    return (() => {
                                        let asc1, end1;
                                        const result = [];
                                        for (idx = 1, end1 = parts.length - 1, asc1 = 1 <= end1; asc1 ? idx <= end1 : idx >= end1; asc1 ? idx++ : idx--) {
