...
window.initTimeGraph = function(def) {
    ...
    window.timeDeferred = timeProxy.makeRequest()
        .fail(error => $("#time_graph").html("<i>Could not draw graph due to a backend error.</i>")).done(function(...args) {
            ...
            return $("#time_graph,#rest_time_graph").bind("plothover", _.throttle(function(event, pos, item) {
                if (item) {
                    ...
                    const pTmpl = _.template("<p><span rel='localize[<%= loc %>]'></span>: <%= num %> <span rel='localize[corpselector_tokens]' </p>")
