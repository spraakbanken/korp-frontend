...
korpApp.directive("mapCtrl", () =>
    ({
        controller($scope, $rootScope, $location, $timeout, searches, nameEntitySearch, markers, nameMapper) {
            ...
            $(document).keyup(function(event) {
                if ((event.keyCode === 27) && s.showMap && s.loading) {
                    ...
                    return $timeout((function() {
                        s.aborted = true;
                        return s.loading = false;}), 0);
                }
            });

            s.$watch((() => $location.search().result_tab), val => $timeout((() => s.tabVisible = val === 1), 0));
            ...
            s.$watch((() => $location.search().show_map), function(val) {
                ...
                if (s.showMap) {
                    ...
                    if ((currentCqp !== (s.lastSearch != null ? s.lastSearch.cqp : undefined)) || (searchCorpora !== (s.lastSearch != null ? s.lastSearch.corpora : undefined))) {
       ...
       <div><span>{{ 'map_rel_occurrences' | loc }}: </span> <span>{{values.rel_occurrences}}</span></div>
            ...
            s.countCorpora = () => __guard__(s.proxy != null ? s.proxy.prevParams : undefined, x => x.corpus.split(",").length);
...
korpApp.directive("newMapCtrl", ($timeout, searches) =>
    ({
        controller($scope, $rootScope) {
            ...
            s.promise.then(((...args) => {
                    const [result] = Array.from(args[0]), xhr = args[1];
                    s.loading = false;
                    s.numResults = 20;
            ...
            return s.newKWICSearch = function(marker) {
                ...
                return $timeout(() => $rootScope.kwicTabs.push({ readingMode: queryData.label === "paragraph__geocontext", queryParams: opts })
                , 0);
            };
        }
    })
