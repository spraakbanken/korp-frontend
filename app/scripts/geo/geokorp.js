  ...
  angular.module('sbMap', ['sbMapTemplate']).filter("trust", function($sce) {
   ...
   <div><span>{{ 'map_rel_occurrences' | loc }}: </span> <span>{{point.rel | number:2}}</span></div>
    ...
    cluster) {
          ...
          return createCircleMarker(color,
    10,
    cluster ? 1 : 5);
        };
    ...
    results;
            ...
            for (i = 0, len = markerData.length; i < len; i++) {
              ...
              results.push([diameter,
    '<div class="geokorp-multi-marker" style="border-radius:' + diameter + 'px;height:' + diameter + 'px;width:' + diameter + 'px;background-color:' + color + '"></div>']);
            }
            return results;
          })();
          elements.sort(function(element1,
    element2) {
    ...
    results;
          ...
          for (idx = i = 0, ref = center; (0 <= ref ? i <= ref : i >= ref); idx = 0 <= ref ? ++i : --i) {
            ...
            for (something = j = 0, ref1 = stop; (0 <= ref1 ? j <= ref1 : j >= ref1); something = 0 <= ref1 ? ++j : --j) {
              if (x === -1) {
                x = center + idx;
              } else {
                x = x + xOp(1);
    ...
    0);
              ...
              results.push('<div class="' + markerClass + '" style="text-align: center;line-height: 0;">' + _.map(row,
    function(elem) {
                return elem[1];
              }).join('') + '</div>');
            }
    ...
    restColor)
          ...
          markerCluster.on('clustermouseover',
    function(e) {
            return mouseOver(_.map(e.layer.getAllChildMarkers(),
    function(layer) {
    ...
    function(e) {
          ...
          markerCluster.on('clusterclick',
    function(e) {
            scope.selectedMarkers = _.map(e.layer.getAllChildMarkers(),
    function(layer) {
              return layer.markerData;
            });
    ...
    results;
                ...
                markerData.sort(function(markerData1,
    markerData2) {
    ...
    function() {
              ...
              hoverInfoElem.css('opacity',
    '1');
              return hoverInfoElem.css('display',
    'block');
            });
          }),
    0);
    ...
    function(e) {
        ...
        scope.$watchCollection("selectedGroups",
    function(selectedGroups) {
          return updateMarkers();
        });
        scope.$watch('useClustering',
    function(newVal,
    oldVal) {
          if (newVal === !oldVal) {
            return updateMarkers();
          ...
          if (scope.useClustering || scope.oldMap) {
            for (j = 0, len1 = selectedGroups.length; j < len1; j++) {
              ...
              for (k = 0, len2 = ref.length; k < len2; k++) {
                ...
                marker = L.marker([markerData.lat,
    markerData.lng],
    {
                  icon: createMarkerIcon(color,
    !scope.oldMap && selectedGroups.length !== 1)
                });
    ...
    {});
        ...
        scope.map.setView([scope.center.lat,
    scope.center.lng],
    scope.center.zoom);
